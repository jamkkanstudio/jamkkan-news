...
운영 스키마, OIDC, 서버 전용 Secrets, owner-scoped 저장 활성화와 문서 커밋 `493c1ec`의 `origin/main` 배포가 완료됐습니다. 푸시 뒤 배포 앱에서 익명 로그인 안내, 공개 브리핑, legacy 성장 기록을 다시 확인했고 계정 B의 개인 기록은 익명 화면에 노출되지 않았습니다. 뉴스 관리 직접 접근도 차단됐고 쓰기 컨트롤은 노출되지 않았습니다.
