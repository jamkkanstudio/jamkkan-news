...
- 초기 주기는 매시 7분부터 10분 간격입니다. 최신 피드 한 번만 요청해 하루 최대 144회로 제한하고 정각 혼잡을 피합니다. 최초 등록 뒤 예약 이벤트가 생성되지 않아 cron 변경 커밋으로 예약 주체와 등록을 갱신했으며, GitHub 예약은 지연될 수 있으므로 준실시간 목표이고 정확한 실행 시각을 보장하지 않습니다.
...
코드와 운영 Secret 이름 네 개의 안전한 UI 설정, 수동 dispatch, 최신 뉴스 노출, 익명 읽기와 관리자 경계 검증은 완료했습니다. 워크플로는 GitHub API에서 `active`이고 수동 실행은 성공합니다. 07:33 KST에 비활성화·재활성화한 뒤에도 예약 이벤트가 없어 커밋 `961d873`에서 cron을 매시 7분부터 10분 간격으로 변경해 예약 주체와 등록을 갱신했습니다. GitHub Actions 허용 설정과 서비스 상태가 정상인 가운데 14:57·15:07 KST 슬롯 뒤에도 `schedule` 이벤트는 0건이었습니다. GitHub 예약 실행이 실제로 생성되고 성공하는 것을 확인하거나 별도 스케줄러 도입을 결정할 때까지 Sprint 상태를 배포 검증 대기로 유지합니다.
